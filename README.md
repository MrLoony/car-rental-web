...
Admin POST actions provide clear one-time feedback after redirects. Examples include booking status updates, car archive/restore, availability toggles, car create/edit actions including image changes, logout, and expired prefill cleanup. Inline validation errors are still shown directly on forms when a form needs correction.

## User Experience Improvements

The application includes small SSR-focused user experience improvements without introducing a SPA or frontend framework:

- Flash messages for admin actions after redirects.
- Custom 404 page for missing or unavailable public/admin resources.
- Custom 500 page for unexpected internal errors.
- Clearer validation messages for booking, login, car create/edit, and upload forms.
- Consistent inline form error styling.
- Clear admin feedback for booking status updates, archive/restore, availability toggles, car create/edit actions, logout, and cleanup actions.

Flash messages are one-time session-backed messages. Inline validation messages remain form-specific and are used when a form is re-rendered after validation fails.

## Error Handling

Missing resources render a custom 404 page. This includes unknown public routes, unavailable or archived public car URLs, and admin resources that cannot be found.

Unexpected internal failures render a custom 500 page with generic user-facing copy. The original error is logged server-side and is not shown in the browser, so SQL errors, repository/service details, and Go internals are not exposed to users.

Bad requests, CSRF failures, validation errors, and authentication/authorization behavior remain separate from the custom 404/500 flow.
...
Upload validation errors are shown with customer/admin-friendly wording. Invalid file types, mismatched file content, empty files, and oversized uploads are rejected without exposing internal validation details.
...
- Stage 19: Email Notifications - complete.
- Stage 20: Flash messages, custom error pages, and form/admin UX polish - complete.

## Stage 20 Smoke Test Checklist

Flash messages:

1. Log in as admin and confirm the success message appears once.
2. Log out and confirm the logout success message appears once.
3. Update a booking status and confirm the success message appears after redirect.
4. Archive and restore a car and confirm the corresponding success messages appear.
5. Toggle car availability and confirm the enabled/disabled message is clear.
6. Run the expired prefill cleanup action from the admin dashboard and confirm the success message appears.

404 page:

1. Open an unknown route and confirm the custom 404 page renders.
2. Open an unknown car slug and confirm the custom 404 page renders.
3. Open an unknown admin car or booking detail URL after login and confirm the custom 404 page renders.

500 page:

1. Confirm `web/templates/pages/server_error.html` exists.
2. Trigger a representative internal error in a development-only/manual scenario if practical and confirm the page shows generic copy, not internal error details.
3. Confirm the real error is logged server-side.

Validation UX:

1. Submit the booking form with missing or invalid fields and confirm readable inline errors appear.
2. Submit invalid login credentials and confirm the error is clear and not duplicated.
3. Upload an invalid car image and confirm the message explains the allowed image formats.
4. Submit invalid admin car form values, such as a non-positive price or seat count, and confirm readable inline errors appear.
...
- Admin action flash feedback
- Custom 404 page
- Custom 500 page
- Form and admin UX polish
